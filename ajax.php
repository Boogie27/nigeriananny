...
if(Input::post('hire_worker'))
{
    ...
    if(!$subscription)
    {
        return response(['subscribe' => ['subscribe' => '*Subscribe to be able to hire a worker']]);
    }
    ...
    $validation = $validate->validate([
        ...
        'address' => 'required|min:6|max:400',
    ]);
  
    

    if(!empty(Input::get('phone')) && !is_numeric(Input::get('phone')))
    {
        return response(['error' => ['phone' => '*invalid phone number']]);
    }
    ...
    $request_worker = $connection->select('request_workers')->where('j_employer_id', Auth_employer::employer('id'))->where('r_worker_id', Input::get('worker_id'))->where('is_accept', 0)->first();
    if($request_worker)
    {
        return response(['hired' => ['hired' => '*This employee has been hired by you!']]);
    }

    $employee = $connection->select('workers')->where('worker_id', Input::get('worker_id'))->first();
    if(!$employee){
        return response(['data' => $data]);
    }
    ...
    $create = $connection->create('request_workers', [
                ...
                'r_worker_id' => Input::get('worker_id'),
                ...
                'j_message' => Input::get('message'),
                'j_address' => Input::get('address'),
